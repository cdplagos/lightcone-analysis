...
def plot_numbercounts(plt, outdir, obsdir, ncounts, ncounts_nod):
    ...
    for subplot, idx, b in zip(subplots, idx, bands):
        ...
        if(idx == 4):
            ...
            y = ncounts[0,b,ind]
            ax.plot(xlf_obs[ind],y[0],'k', linewidth=3, label='Shark r<27')
 
            ind = np.where(ncounts_nod[0,b,:] != 0)
            y = ncounts_nod[0,b,ind]
            ax.plot(xlf_obs[ind],y[0],'k', linewidth=1, label='Shark total')
    ...
    common.savefig(outdir, fig, "number-counts-deep-lightcone-optical-lsst.pdf")
    ...
    common.savefig(outdir, fig, "number-counts-deep-lightcone-IR-lsst.pdf")
    ...
    common.savefig(outdir, fig, "number-counts-deep-lightcone-FIR-lsst.pdf")

def prepare_data(phot_data, phot_data_nod, ids_sed, hdf5_data, subvols, lightcone_dir, ncounts, nbands, ncounts_nodust):
    ...
    for i in range(0,nbands):
        ...
        ind = np.where((SEDs_dust[i,:] > 0) & (SEDs_dust[4,:] < 27))
...
def main():
    ...
    prepare_data(seds, seds_nod, ids_sed, hdf5_data, subvols, lightcone_dir, ncounts, nbands, ncounts_nodust)
