...
def _copy_subvolume(f_in, output_fname, subvol):
    ...
    selection = np.where(galaxies['subvolume'].value == subvol)[0].tolist()
    if not selection:
        logger.warning('No data found for subvolume %d', subvol)
        return
    logger.info('Selection for subvolume %d: %d elements (%.2f %% of total)',
                subvol, len(selection), 100. * len(selection) / len(galaxies['subvolume'].value))
    ...
    with h5py.File(output_fname, 'w') as f_out:
        ...
        for group in ('run_info', 'parameters'):
            ...
            for dataset in original_group:
                ...
                f_out[group][dataset] = original_group[dataset].value[:]
        ...
        for dataset in galaxies:
            ...
            f_out['galaxies'][dataset] = galaxies[dataset].value[selection][:]
...
def main():
    ...
    output_fname_pattern = os.path.join(opts.output_dir, bname + "_%02d.hdf5")
